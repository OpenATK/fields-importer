...
var request = require('superagent'); // TODO: URIjs is big, find smaller alternative
...
function mergeOptions() {
  ...
  options.scope = args.map(function (arg) {
...
function storeState(stateObj, callback) {
  ...
  var stateTok = crypto.randomBytes(16); // Make it base64URL

  stateTok = stateTok.toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, ''); // Rembmer stuff
...
function retrieveState(stateTok, callback) {
  ...
  var stateObj = stuff[stateTok]; // Only use each state once
...
function authorize(domain, configuration, parameters, redirect, callback) {
  var key;
  var params;
  var metadata; // Get stuff from options object

  var options = mergeOptions({}, options, parameters);
  key = options.privateKey;
  params = objectAssign(options.params, {
    scope: options.scope
  });
  metadata = options.metadata; // Assume key is PEM ecnoded
  ...
  req.end(function configurationCallback(err, resp) {
    ...
    try {
      ...
      register(metadata, conf['registration_endpoint'], function registrationCallback(err, resp) {
        if (err) {
          return errCallback(err);
        } // Is this a good way to pick?


        params['redirect_uri'] = options.redirect || resp['redirect_uris'][0]; // Stuff to remember for when redirect is received
        ...
        core.storeState(stateObj, function (err, stateTok) {
          ...
          uri.addQuery({
            state: stateTok
          }).addQuery(params).addQuery({
            client_id: resp['client_id']
          }); // Do not send client_secret here

          uri.removeQuery('client_secret'); // Redirect the user to constructed uri
...
core.getIDToken = function getIDToken(domain, opts, redirect, cb) {
  ...
  var response = process.browser ? 'id_token' : 'code'; // Make sure we have openid scope
  ...
  params.params.nonce = crypto.randomBytes(12).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
...
function verifyIDToken(state, params, callback) {
  ...
  return jwku.jwkForSignature(parameters['id_token'], state.conf['jwks_uri'], function (err, jwk) {
    if (err) {
      return callback(err);
    }
...
function exchangeCode(state, parameters, callback) {
  ...
  var assertion = clientAuth.generate(state.key, state.options['client_id'], state.options['client_id'], state.conf['token_endpoint'], 60, {
    payload: {
      jti: parameters.code
    }
  });
  ...
  request.post(state.conf['token_endpoint']).type('form').send(params).end(function (err, resp) {
    var e = err || resp.error;
